...
static irqreturn_t ds_platform_interrupt_handler(int irq, void *dev_id);
...
static int ds_platform_register_handler(struct ds_dev *ds_dev)
{
	...
	BUG_ON(!ds_dev);
	BUG_ON(!ds_dev->interrupt_data);

	dev = ds_get_device(ds_dev);
	irq = platform_get_irq(ds_dev->platform_dev, 0);
	if (irq < 0) {
		ds_nodev_error("Couldnt request irq\n");
		return -1;
	}

	ret = devm_request_irq(dev, irq, ds_platform_interrupt_handler,
		IRQF_SHARED, "knoll", ds_dev);
	if (ret) {
		ds_nodev_error("Cannot get IRQ %d; %d\n", irq, ret);
		return ret;
	} else {
		ds_nodev_error("Requested IRQ %d; %s\n", irq, "knoll");
	}
	ds_dev->interrupt_data->irq = irq;
...
static void ds_platform_free_handler(struct ds_dev *ds_dev)
{
	...
	devm_free_irq(dev, ds_dev->interrupt_data->irq, ds_dev);
	...
	struct ds_dev *dev, struct ds_interrupt_data *interrupt_data)
{
	...
	ret = pci_enable_msi_block(dev->pci_dev, 32);
#else
	ret = pci_enable_msi_range(dev->pci_dev, 32, 32);
#endif
	if (ret != 0) {
	...
	for (i = 0; i < interrupt_data->num_interrupts; i++) {
		...
		if (interrupt_data->interrupts[i].packing == UNPACKED) {
			...
			case PACK_3:
				...
				break;
			default:
				ds_nodev_error("Found interrupt description with unknown enum %d.",
					interrupt_data->interrupts[i].packing);
				return;
			}
...
void ds_interrupt_pause(struct ds_dev *ds_dev, int enable_pause)
{
	...
	BUG_ON(ds_dev->interrupt_data->type != PLATFORM_WIRE);

	if (enable_pause && ds_dev->interrupt_data->init_complete) {
		BUG_ON(!ds_dev->interrupt_data->wire_interrupt_offsets);
		BUG_ON(!ds_dev->interrupt_data->wire_interrupt_offsets
		       ->mask_array);
...
static irqreturn_t ds_platform_interrupt_handler(int irq, void *dev_id)
{
	...
	u64 pending_bits;
	u64 masked_bits;
	int interrupt = 0; /* platform driver has only one interrupt line */
