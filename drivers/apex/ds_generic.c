...
const struct ds_num_name ds_status_name_table[] = { { DS_STATUS_DEAD, "DEAD" },
	{ DS_STATUS_ALIVE, "ALIVE" }, { DS_STATUS_LAMED, "LAMED" },
	{ 0, NULL } };
 ...
 */
#define DW_TN_BAR_INDEX 2
	r = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	if (r == NULL) {
		dev_err(&pdev->dev, "dwn_platform_driver_probe "
				    "cannot get mem resource\n");
		return -ENODEV;
	}
	...
	dev->bar_data[DW_TN_BAR_INDEX].virt_base =
		ioremap_nocache(dev->bar_data[DW_TN_BAR_INDEX].phys_base,
			dev->bar_data[DW_TN_BAR_INDEX].length_bytes);
...
static int dwn_platform_driver_remove(struct platform_device *pdev)
{
	...
	ds_nodev_info("dwn_platform_driver_remove %p.", pdev);
	...
	ds_nodev_info("dwn_platform_driver_remove %s platform device %s",
		internal_desc->driver_desc->name, dev->kobj_name);
	...
	struct device *parent, struct ds_dev **pdev, const char *kobj_name)
{
	...
	*pdev = NULL;
	...
	ds_nodev_info("num_page_tables: %d\n", driver_desc->num_page_tables);
	...
	struct ds_internal_desc *internal_desc, struct ds_dev *dev)
{
	...
	for (i = 0; i < driver_desc->num_page_tables; i++) {
		...
		if (dev->pci_dev) {
			ddev = &(dev->pci_dev->dev);
		} else if (dev->platform_dev) {
			ddev = &(dev->platform_dev->dev);
		} else {
			ds_log_error(
				dev, "ds_enable_dev with no physical device!!");
			BUG_ON(1);
			ddev = NULL;
		}
...
static int ds_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (!ds_dev) {
		ds_nodev_error("Unable to retrieve jf device data");
	...
	permissions = driver_desc->bar_descriptions[bar_index].permissions;
	...
	if (bar_desc->type == COHERENT_MEMORY) {

		if (requested_length == 0 ||
			requested_length > bar_data->length_bytes) {
			trace_ds_mmap_exit(-EINVAL);
			return -EINVAL;
		}
		...
		ret = remap_pfn_range(vma, vma->vm_start,
			(bar_data->phys_base) >> PAGE_SHIFT, requested_length,
			vma->vm_page_prot);
		if (ret) {
			ds_log_error(ds_dev,
		...
		ds_set_user_virt(ds_dev, requested_length, bar_data->phys_base,
			vma->vm_start);
	} else {
...
/* ds_get_device: Get the device stucture for a given ds_dev
 * @dev: pointer to ds_dev, implementing the requested driver.
 */
