...
struct cros_ec_extcon_info {
	...
	unsigned int vconn; /* vconn enabled */
...
enum {
	...
	EXTCON_CABLE_TYPEC_POLARITY,
	EXTCON_CABLE_TYPEC_PIN_ASSIGN,
	EXTCON_CABLE_DISP_DP,
};

#define EXTCON_CABLE_STATE_MASK	(BIT(EXTCON_CABLE_USB) | \
				 BIT(EXTCON_CABLE_USB_HOST) | \
				 BIT(EXTCON_CABLE_TYPEC_POLARITY) | \
				 BIT(EXTCON_CABLE_TYPEC_PIN_ASSIGN) | \
				 BIT(EXTCON_CABLE_DISP_DP))
				       ...
				       bool force)
{
	...
	int role, power_type;
	unsigned int vconn, dr, pr;
	bool polarity, usb_pd, dp, mux;
	...
	role = cros_ec_usb_get_role(info, &polarity, &usb_pd);
	if (role < 0) {
		if (role != -ENOTCONN) {
		...
		vconn = !!(role & PD_CTRL_RESP_ROLE_VCONN);
	...
	if (info->dr != dr || info->pr != pr || info->dp != dp ||
		...
		/*
		 * Note:
		 * EC currently only supports pin assignments C and D.
		 * Reflect this by always setting EXTCON_CABLE_TYPEC_PIN_ASSIGN.
		 */
		extcon_update_state(info->edev,
			EXTCON_CABLE_STATE_MASK,
			(device_connected ? BIT(EXTCON_CABLE_USB) : 0) |
			(host_connected ? BIT(EXTCON_CABLE_USB_HOST) : 0) |
			(polarity ? BIT(EXTCON_CABLE_TYPEC_POLARITY) : 0) |
			(dp ? BIT(EXTCON_CABLE_DISP_DP) : 0) |
			BIT(EXTCON_CABLE_TYPEC_PIN_ASSIGN));
