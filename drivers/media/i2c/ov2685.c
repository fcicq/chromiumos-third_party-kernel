...
#define to_ov2685(sd) container_of(sd, struct ov2685_priv, subdev)
...
static struct ov2685_reg ov2685_globe_setting[] = {
	{0x0103, 0x01},
	{0x3002, 0x00},
	{0x3016, 0x1c},
	{0x3018, 0x44},
...
static struct ov2685_reg ov2685_1600x900_regs[] = {
	{0x3086, 0x00},
	{0x3501, 0x48},
	{0x3502, 0xe0},
	{0x370a, 0x21},
...
static struct ov2685_reg ov2685_1280x960_regs[] = {
	{0x3086, 0x00},
	{0x3501, 0x48},
	{0x3502, 0xe0},
	{0x370a, 0x21},
...
static struct ov2685_reg ov2685_1280x720_regs[] = {
	{0x3086, 0x00},
	{0x3501, 0x48},
	{0x3502, 0xe0},
	{0x370a, 0x21},
...
static const struct ov2685_mode supported_modes[] = {
	...
	},
	{
		.width = 1600,
		.height = 900,
		.reg_list = ov2685_1600x900_regs,
	},
	{
		.width = 1280,
		.height = 960,
		.reg_list = ov2685_1280x960_regs,
	},
	{
		.width = 1280,
		.height = 720,
		.reg_list = ov2685_1280x720_regs,
	},
};
...
static int ov2685_poweron(struct ov2685_priv *priv)
{
	int ret;
...
static void ov2685_poweroff(struct ov2685_priv *priv)
{
...
{
	struct ov2685_priv *priv = to_ov2685(sd);
	int ret = 0;
	...
	if (on)
		ret = ov2685_poweron(priv);
	else
		ov2685_poweroff(priv);
...
static int ov2685_get_reso_dist(const struct ov2685_mode *mode,
...
static const struct ov2685_mode *ov2685_find_best_fit(
					struct v4l2_subdev_format *fmt)
{
	...
	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		dist = ov2685_get_reso_dist(&supported_modes[i], framefmt);
...
static int ov2685_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		ret = ov2685_write_array(client, ov2685_globe_setting);
		if (ret)
			goto unlock_and_return;
		/* 0.2ms is the max PLL startup/lock time */
		usleep_range(200, 250);
		ret = ov2685_write_array(client, priv->cur_mode->reg_list);
		if (ret)
			goto unlock_and_return;
...
static int ov2685_initialize_controls(struct ov2685_priv *priv)
{
...
static int ov2685_check_sensor_id(struct ov2685_priv *priv,
			      struct i2c_client *client)
{
	struct device *dev = &priv->client->dev;
	u8 id_msb, id_lsb;
			...
			const struct i2c_device_id *id)
{
	...
	ret = ov2685_check_sensor_id(priv, client);
	if (ret)
		return ret;

	mutex_init(&priv->mutex);
	v4l2_i2c_subdev_init(&priv->subdev, client, &ov2685_subdev_ops);
	ret = ov2685_initialize_controls(priv);
	if (ret)
		goto destroy_mutex;

	priv->pad.flags = MEDIA_PAD_FL_SOURCE;
	priv->subdev.internal_ops = &ov2685_internal_ops;
...
{
	struct ov2685_priv *priv = i2c_get_clientdata(client);
