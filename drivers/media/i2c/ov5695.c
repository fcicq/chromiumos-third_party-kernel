...
#define OV5695_REG_DIGI_GAIN_MSB	0x350a
#define OV5695_REG_DIGI_GAIN_LSB	0x350b
...
static int ov5695_write_reg(struct i2c_client *client, u16 reg, u8 val)
{
	struct i2c_msg msg;
	u8 buf[3];
	int ret;
			      ...
			      const struct regval *regs)
{
	...
	for (i = 0; ret == 0 && regs[i].addr != REG_NULL; i++)
		ret = ov5695_write_reg(client, regs[i].addr, regs[i].val);
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		*v4l2_subdev_get_try_format(sd, cfg, fmt->pad) = fmt->format;
		...
		__v4l2_ctrl_modify_range(ov5695->hblank, h_blank,
					 h_blank, 1, h_blank);
		__v4l2_ctrl_modify_range(ov5695->vblank,
					 ov5695->vblank->minimum,
					 ov5695->vblank->maximum, 1,
...
static int ov5695_enable_test_pattern(struct ov5695 *ov5695, u32 pattern)
{
	...
	return ov5695_write_reg(ov5695->client, OV5695_REG_TEST_PATTERN, val);
}

static void ov5695_set_exposure(struct ov5695 *ov5695, s32 val)
{
	ov5695_write_reg(ov5695->client, 0x3502, val & 0xff);
	ov5695_write_reg(ov5695->client, 0x3501, (val >> 8) & 0xff);
	ov5695_write_reg(ov5695->client, 0x3500, (val >> 16) & 0x0f);
}

static void ov5695_set_analog_gain(struct ov5695 *ov5695, s32 val)
{
	ov5695_write_reg(ov5695->client, OV5695_REG_ANALOG_GAIN, val & 0xff);
}

static void ov5695_set_digi_gain(struct ov5695 *ov5695, s32 val)
{
	ov5695_write_reg(ov5695->client, OV5695_REG_DIGI_GAIN_LSB, val & 0x3f);
	val >>= 6;
	ov5695_write_reg(ov5695->client, OV5695_REG_DIGI_GAIN_MSB, val & 0xff);
}
...
static void ov5695_set_vts(struct ov5695 *ov5695, s32 val)
{
	...
	ov5695_write_reg(ov5695->client, 0x380e, (val >> 8) & 0xff);
	ov5695_write_reg(ov5695->client, 0x380f, val & 0xff);
}
...
static int __ov5695_power_on(struct ov5695 *ov5695)
{
	...
	ret = clk_prepare_enable(ov5695->mclk);
	if (ret < 0) {
...
disable_clk:
	clk_disable_unprepare(ov5695->mclk);
...
static int ov5695_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_EXPOSURE:
		...
		ov5695_set_exposure(ov5695, ctrl->val << 4);
		break;
...
static int ov5695_initialize_controls(struct ov5695 *ov5695)
{
	...
	/* TODO:
	pixel_rate = (link_freq_menu_items[0] * 2 * OV5695_LANES) /
		     OV5695_BITS_PER_SAMPLE;
	*/
	pixel_rate = 45 * 1000 * 1000 * 4;
	...
	ov5695->vblank = v4l2_ctrl_new_std(handler, &ov5695_ctrl_ops,
				V4L2_CID_VBLANK, OV5695_VBLANK_MIN,
				OV5695_VTS_MAX - mode->height, 1,
				mode->vts_def - mode->height);
	...
	v4l2_ctrl_new_std(handler, &ov5695_ctrl_ops, V4L2_CID_ANALOGUE_GAIN,
			  ANALOG_GAIN_MIN, ANALOG_GAIN_MAX, ANALOG_GAIN_STEP,
			  ANALOG_GAIN_DEFAULT);

	/* Digital gain */
	v4l2_ctrl_new_std(handler, &ov5695_ctrl_ops, V4L2_CID_DIGITAL_GAIN,
			  OV5695_DIGI_GAIN_MIN, OV5695_DIGI_GAIN_MAX,
			  OV5695_DIGI_GAIN_STEP, OV5695_DIGI_GAIN_DEFAULT);

	v4l2_ctrl_new_std_menu_items(handler, &ov5695_ctrl_ops,
			...
			const struct i2c_device_id *id)
{
	...
	ov5695->mclk = devm_clk_get(dev, "mclk");
	if (IS_ERR(ov5695->mclk)) {
		dev_err(dev, "Failed to get mclk\n");
		return -EINVAL;
	}
	ret = clk_set_rate(ov5695->mclk, 24000000);
	if (ret < 0) {
