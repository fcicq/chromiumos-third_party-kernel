					...
					struct device_node *of_node)
{
	...
	if (rc < 0) {
		pr_err("%s write register %x failed rc = %d\n",
			__func__, REG_LAB_PRECHARGE_CTL, rc);
		return rc;
	}
	...
	if (!(val & LAB_MODULE_RDY_EN)) {
		...
		if (rc < 0) {
			pr_err("%s write register %x failed rc = %d\n",
				__func__, REG_LAB_MODULE_RDY, rc);
			return rc;
		}
	}
				...
				struct device_node *of_node)
{
	...
	if (of_property_read_bool(of_node,
		...
		if (rc < 0) {
			pr_err("%s write register %x failed rc = %d\n",
				__func__, REG_IBB_RING_SUPPRESSION_CTL, rc);
			return rc;
		}
	}
	...
	if (of_property_read_bool(of_node, "qcom,qpnp-ibb-ps-enable")) {
		rc = qpnp_ibb_ps_config(labibb, true);
		if (rc < 0) {
			pr_err("%s PS enable failed rc=%d\n", __func__, rc);
			return rc;
		}
	} else {
		rc = qpnp_ibb_ps_config(labibb, false);
		if (rc < 0) {
			pr_err("%s PS disable failed rc=%d\n", __func__, rc);
			return rc;
		}
	}
	...
	if (of_property_read_bool(of_node,
		...
		if (rc < 0) {
			pr_err("%s smart PS enable failed rc=%d\n",
							__func__, rc);
			return rc;
		}
					...
					struct device_node *of_node)
{
	...
	if (!(val & IBB_MODULE_RDY_EN)) {
		...
		if (rc < 0) {
			pr_err("%s write register %x failed rc = %d\n",
				__func__, REG_IBB_MODULE_RDY, rc);
			return rc;
		}
	}
