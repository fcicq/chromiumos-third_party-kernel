					...
					unsigned int enabled)
{
	...
	if (enabled) {
		...
		if (alarm_value < current_time)
			...
			cros_ec_rtc->saved_alarm = alarm_value - current_time;
...
{
	struct cros_ec_device *cros_ec = dev_get_drvdata(pdev->dev.parent);
	struct cros_ec_rtc *cros_ec_rtc;
	struct rtc_time tm;
	int ret;
	...
	cros_ec_rtc->rtc = devm_rtc_device_register(&pdev->dev, "cros-ec-rtc",
						  &cros_ec_rtc_ops,
						  THIS_MODULE);
...
#ifdef CONFIG_OF
static const struct of_device_id cros_ec_rtc_of_match[] = {
	{ .compatible = "google,cros-ec-rtc", },
	{ /* sentinel */ },
};
MODULE_DEVICE_TABLE(of, cros_ec_rtc_of_match);
#endif
...
static struct platform_driver cros_ec_rtc_driver = {
	...
	.driver = {
		.name = "cros-ec-rtc",
		.of_match_table = of_match_ptr(cros_ec_rtc_of_match),
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:cros-ec-rtc");
