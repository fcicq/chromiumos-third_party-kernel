...
static DECLARE_HASHTABLE(package_to_appid, 8);
static DECLARE_HASHTABLE(package_to_userid, 8);
static DECLARE_HASHTABLE(ext_to_groupid, 8);
				...
				const char *page, size_t count)
{
	...
	ret = insert_packagelist_entry(&package_details->name, uid);
...
static ssize_t package_details_excluded_userids_show(
			struct package_details *package_details, char *page)
{
	struct hashtable_entry *hash_cur;
...
static ssize_t package_details_excluded_userids_store(
			struct package_details *package_details,
			const char *page, size_t count)
{
	unsigned int tmp;
...
static ssize_t package_details_clear_userid_store(
			struct package_details *package_details,
			const char *page, size_t count)
{
	unsigned int tmp;
...
PACKAGE_DETAILS_ATTR(appid, S_IRUGO | S_IWUGO, package_details_appid_show,
			package_details_appid_store);
PACKAGE_DETAILS_ATTR(excluded_userids, S_IRUGO | S_IWUGO,
		package_details_excluded_userids_show,
		package_details_excluded_userids_store);
PACKAGE_DETAILS_ATTR(clear_userid, S_IWUGO, NULL,
		package_details_clear_userid_store);
...
CONFIGFS_ATTR_STRUCT(extensions_value);
#define EXTENSIONS_VALUE_ATTR(_name, _mode, _show, _store)	\
static struct extensions_value_attribute extensions_value_attr_##_name = \
			__CONFIGFS_ATTR(_name, _mode, _show, _store)
#define EXTENSIONS_VALUE_ATTRIBUTE(name) (&extensions_value_attr_##name.attr)
...
static ssize_t extensions_value_extgid_show(
			struct extensions_value *value, char *page)
{
	return scnprintf(page, PAGE_SIZE, "%u\n",
				from_kgid(current_user_ns(), value->gid));
}

static ssize_t extensions_value_extgid_store(
				struct extensions_value *value,
				const char *page, size_t count)
{
	unsigned int tmp;
	...
	value->gid = make_kgid(current_user_ns(), tmp);

	return count;
}

EXTENSIONS_VALUE_ATTR(ext_gid, S_IRUGO | S_IWUGO, extensions_value_extgid_show,
		extensions_value_extgid_store);
...
CONFIGFS_ATTR_OPS(extensions_value)
static struct configfs_item_operations extensions_value_item_ops = {
	.show_attribute = extensions_value_attr_show,
	.store_attribute = extensions_value_attr_store,
};

static struct config_item_type extensions_name_type = {
	.ct_item_ops	= &extensions_value_item_ops,
...
static struct packages_attribute packages_attr_##_name = \
			__CONFIGFS_ATTR(_name, _mode, _show, _store)
...
static struct packages_attribute packages_attr_packages_gid_list =
		__CONFIGFS_ATTR_RO(packages_gid.list, packages_list_show);
