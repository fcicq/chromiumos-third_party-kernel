 ...
 */
static inline unsigned long get_available_anon_mem(int lru_base)
{
	return global_page_state(lru_base + LRU_ACTIVE_ANON) +
...
static inline unsigned long get_available_mem_adj(int lru_base)
{
	...
	unsigned long swappable_pages = min_t(unsigned long,
			get_nr_swap_pages(), get_available_anon_mem(lru_base));
	/*
...
static inline bool _is_low_mem_situation(void)
{
	...
	if (unlikely(is_low_mem && !was_low_mem)) {
		unsigned long anon_mem = get_available_anon_mem(lru_base);
